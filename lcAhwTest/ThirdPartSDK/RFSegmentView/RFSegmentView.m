...
#define kDefaultTintColor   RGB_Color(204.0, 204.0, 206.0) 
#define kLeftMargin         0.0   // 15 -> 0.0
//#define kItemHeight         30
#define kBorderLineWidth    1.0

// 选中色和非选中色
#define kSelectedColor        RGB_Color(255.0, 255.0, 255.0)
#define kUnSelectedColor      RGB_Color(244.0, 245.0, 249.0)
...
- (id)initWithFrame:(CGRect)frame index:(NSInteger)index title:(NSString *)title norColor:(UIColor *)norColor selColor:(UIColor *)selColor isSelected:(BOOL)isSelected;
{
    ...
    if (self) {
        ...
        self.norTitleColor   = [UIColor grayColor];
...
- (void)setSelColor:(UIColor *)selColor
{
    if (_selColor != selColor) {
        ...
        if (_isSelected) {
            self.titleLabel.textColor = self.selTitleColor;
            self.backgroundColor = self.selColor;
        }
        else
        {
            self.titleLabel.textColor = self.norTitleColor;
...
//        float init_y          = CGRectGetMinY(frame);
        ...
        //
        self.bgView = [[UIView alloc] initWithFrame:CGRectMake(kLeftMargin, 0.0, viewWidth -2*kLeftMargin, viewHeight)];
        self.bgView.backgroundColor    = [UIColor whiteColor];
        self.bgView.clipsToBounds      = YES;
        self.bgView.layer.cornerRadius = 5;
        self.bgView.layer.borderWidth  = kBorderLineWidth;
        self.bgView.layer.borderColor  = kDefaultTintColor.CGColor;//[UIColor grayColor].CGColor;
        ...
        if (items.count >= 2) {
            for (NSInteger i =0; i<items.count; i++) {
                RFSegmentItem *item = [[RFSegmentItem alloc] initWithFrame:CGRectMake(init_x, init_y, itemWidth, itemHeight)
                                                                  ...
                                                                  norColor:kUnSelectedColor
                                                                  selColor:kSelectedColor
...
{
    for (RFSegmentItem *item in self.itemsArray) {
        item.isSelected = NO;
    }
    RFSegmentItem *item = (RFSegmentItem *)[self.itemsArray objectAtIndex:selectedIndex];
    item.isSelected = YES;
}
