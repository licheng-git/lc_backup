...
- (void)startRequest
{
    ...
    // add by hhx 2014.12.15 去掉函数被废弃的警告
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if ([self PACurl]) {
            ...
            if (proxies && proxies.count > 0) // 添加判断 add by hhx 2014.11.28
            {
...
+ (NSString *)defaultUserAgentString
{
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
            ...
            // 去掉编码转换 delete by hhx 2014.08.29
...
+ (NSDate *)expiryDateForRequest:(ASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
    ...
    // addTimeInterval -> dateByAddingTimeInterval modify by hhx 2014.05.16
	if (maxAge) {
