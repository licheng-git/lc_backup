...
{
    // YES -> (toInterfaceOrientation == UIInterfaceOrientationPortrait) modify 2014.11.27
...
{
    // 添加判断 modify by hhx 2014.11.28
    NSArray *array = [[[[self tableView] cellForRowAtIndexPath:
...
+ (void)dismiss
{
    // ios6废弃方法 modify by hhx 2015.10.21
...
{
    [super viewDidDisappear:animated]; // add by hhx 2014.09.22
...
- (void)showTitle
{
    ...
    {
        navigationBar = [[[self view] subviews] objectAtIndex:0];
    }
    
    if (navigationBar && [navigationBar items].count > 0)
    {
        navItem = [[navigationBar items] objectAtIndex:0];
    }
...
- (void)show
{
    ...
    // 添加判断 modify by hhx 2014.11.28
    NSArray *array = [[[self tableView] cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]].contentView subviews];
    if (array && array.count > 0)
    {
        [[array objectAtIndex:0] becomeFirstResponder];
    }
