...
//    NSLog(@"---touchesEnded---");
    ...
    [touchBeginDelegate gestureTouchEnd:touchesArray];
    ...
    for (int i=0; i<touchesArray.count; i++) {
        ...
        // 防止数组越界 modify by hhx 2015.02.06
        GesturePasswordButton *buttonTemp = nil;
        if (selection < buttonArray.count)
        {
            buttonTemp = ((GesturePasswordButton *)[buttonArray objectAtIndex:selection]);
        }
...
// 获取输入手势密码点数
- (NSInteger)getEnterArginCount
{
    if (touchedArray)
    {
        return touchedArray.count;
    }
