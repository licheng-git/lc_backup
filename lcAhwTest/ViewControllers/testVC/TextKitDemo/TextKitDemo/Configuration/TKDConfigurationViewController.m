...
- (void)viewDidLoad
{
	...
	//[sharedTextStorage replaceCharactersInRange:NSMakeRange(0, 0) withString:[NSString stringWithContentsOfURL:[NSBundle.mainBundle URLForResource:@"lorem" withExtension:@"txt"] usedEncoding:NSUTF8StringEncoding error:NULL]];
    NSURL *url = [[NSBundle mainBundle] URLForResource:@"lorem" withExtension:@"txt"];
    NSString *str = [NSString stringWithContentsOfURL:url encoding:NSUTF8StringEncoding error:nil];
    self.originalTextView.text = str;
	...
	otherTextView.scrollEnabled = NO;  // *_*
...
- (IBAction)endEditing:(UIBarButtonItem *)sender
{
    ...
    NSLog(@"*_* %@", self.otherTextView.textStorage);
    NSLog(@"*_* %@", self.thirdTextView.textStorage);
}
